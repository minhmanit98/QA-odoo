...
class QLD (models.Model):
    ...
    def _compute_tong_stc(self):
        for mon in self:
            tongstc = 0
            for score in mon.scores_ids:
                tongstc += score.subject_id.stc
            mon.tong_stc = tongstc
    ...
    def _compute_scores_4end(self):
        for mon in self:
            mon.scores_4end = mon.diem_tich_luy()

    name = fields.Char('MSV', required=True)
    user_id = fields.Many2one('res.users', string='Student')
    scores_4end = fields.Float(string='Điểm tích lũy', compute=_compute_scores_4end, readonly=True, store=True)
    ...
    def get_diem_tich_luy(self, diem):
    ...
    def get_diem_tich_luy2(self, diem):
